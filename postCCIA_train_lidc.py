...
def main(args):
    ...
    custom_transform = partial(parse_lidc_metadata, resolution=args.resolution)
    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            masks = load_nodule_masks(batch["img_ids"], metadata_path, masks_dir, resolution=args.resolution, device=clean_images.device, bbxmask=False)
            masks = masks.type(weight_dtype)

            latents = vae.encode(clean_images).latents
            latents = latents * 0.18215 #vae.config.scaling_factor
            #latents = torch.cat([latents[:, 0:1, :, :], latents[:, 0:1, :, :], latents[:, 0:1, :, :]], dim=1)
            ...
            with accelerator.accumulate(model):
                ...
                # free cuda memory
                optimizer.zero_grad(set_to_none=True) # frees more memory than just zero_grad()
        ...
        if accelerator.is_main_process:
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                model.eval()
                del batch, latents, noisy_latents, clean_images, masks, mask_latents, cond_latents, timesteps, model_output, noise
                ...
                output = pipeline(
                    ...
                    batch_size=3, #args.eval_batch_size,
